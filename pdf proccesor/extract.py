...
def segment_questions_regex(all_blocks: list[dict]) -> dict:
    ...
    for item in all_blocks:
        ...
        block_rect = rect_from_bbox(b["bbox"])
        
        # Skip empty blocks or blocks in the header/footer margins (prevents huge empty space at the bottom of questions)
        if not text or block_rect.y0 < 50 or block_rect.y0 > 750:
            continue

        match = QUESTION_PATTERN.match(text)

        is_left_anchored = block_rect.x0 < LEFT_MARGIN_THRESHOLD
        ...
        if match and is_left_anchored:
            ...
            # Add to current question on the SAME page
